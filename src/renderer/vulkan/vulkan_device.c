...
b8 vulkan_device_create(VkInstance instance, VkSurfaceKHR surface, vulkan_device *out_device) {
	...
	b8 compute_shares_graphics_queue = out_device->graphics_queue_index == out_device->compute_queue_index;
	b8 present_shares_graphics_queue = out_device->graphics_queue_index == out_device->present_queue_index;
	b8 transfer_shares_graphics_queue = out_device->graphics_queue_index == out_device->transfer_queue_index;
	b8 present_shares_compute_queue = out_device->compute_queue_index == out_device->present_queue_index;
	b8 transfer_shares_compute_queue = out_device->compute_queue_index == out_device->transfer_queue_index;
	u32 index_count = 1;
	...
	if (vkCreateDevice(out_device->physical_device, &create_info, NULL, &out_device->logical_device) != VK_SUCCESS) {
		return false;
	}

	vkGetDeviceQueue(out_device->logical_device, out_device->graphics_queue_index, 0, &out_device->graphics_queue);

	vkGetDeviceQueue(out_device->logical_device, out_device->compute_queue_index, 0, &out_device->compute_queue);

	vkGetDeviceQueue(out_device->logical_device, out_device->present_queue_index, 0, &out_device->present_queue);

	vkGetDeviceQueue(out_device->logical_device, out_device->transfer_queue_index, 0, &out_device->transfer_queue);
...
void vulkan_device_destroy(vulkan_device *device) {
	...
	memset(&device->swapchain_support.capabilities, 0, sizeof(device->swapchain_support.capabilities));
...
void vulkan_device_query_swapchain_support(VkPhysicalDevice physical_device,
	...
	vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_device, surface, &out_support_info->capabilities);

	vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, surface, &out_support_info->format_count, 0);
	...
	if (out_support_info->present_mode_count != 0) {
		if (!out_support_info->present_modes) {
			out_support_info->present_modes = malloc(sizeof(VkPresentModeKHR) * out_support_info->format_count);
